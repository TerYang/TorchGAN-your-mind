...
G = nn.Sequential(                      # Generator,input 1,1,4,1,output 1,64,64,21
    ...
    nn.ConvTranspose2d(in_channels=16, out_channels=4, kernel_size=(4, 5), stride=2, padding=1, output_padding=0,
                       bias=False),
    nn.ReLU(),
    nn.ConvTranspose2d(in_channels=4, out_channels=1, kernel_size=(4, 3), stride=2, padding=1, output_padding=0, bias=False),
    # nn.ReLU(),
    nn.Tanh(),
)
...
if __name__ == '__main__':
    ...
    num_nor, train_all = new_get_norlmal()  # num=64*10000 train_normal,train_anormal
    ...
    for epoch in range(EPOCH):                    # start training
        if epoch == 20:
            opt_G.param_groups[0]['lr'] = 0.00005
        elif epoch == 40:
            opt_G.param_groups[0]['lr'] = 0.00001
        for step, (T,L) in enumerate(train_loader):
            ...
            """modified end"""
            """
            G_generate.shape torch.Size([1, 1, 64, 21])
            l.shape torch.Size([64, 1, 64, 21])
            ...
            opt_G.zero_grad()
            G_loss_D.backward(retain_graph=True)  #retain_graph=True
            # G_loss_D.backward()
            opt_G.step()
            ...
            if step % 200 == 0:
                ...
                # print('Epoch: ', epoch, '|Gloss: %.6f' % (G_loss_D),'|train Dloss: %.6f'%D_loss.item())
                scalar_dir = './scalar'
    ...
    writer.export_scalars_to_json('./allScalar.json')
    writer.close()